...
int vip_sdl_audio_decode_frame(AVCodecContext *aCodecCtx,
                       uint8_t *audio_buf,
                       int buf_size,
                       VIPMediaContext *mCtx,double *pts_ptr){
    AVPacket *pkt = NULL;
    ...
    while(1){
        ...
        *pts_ptr = pts;
        ...
        if(vip_packet_queue_get_packet(mCtx->audioQueue, (void **)&pkt, 1)< 0) {
            return -1;
        }
...
void vip_sdl_audio_callback(void *userdata, Uint8 * stream,int len){
    ...
    //音频回调函数->音频解码->返回一帧数据(音频)
    //SDL自动调用
    //每次调用一次，那么你就解码一帧数据->音频压缩数据
    while(len > 0) {
        if(mCtx->audio_buf_index >= mCtx->audio_buf_size) {
            /* We have already sent all our data; get more */
            audio_size = vip_sdl_audio_decode_frame(codecCtx,
                                            audio_buf,
                                            sizeof(audio_buf),
                                            mCtx,&pts);
            if(audio_size < 0) {
...
VIPSDLContext *vip_sdl_create_context(struct VIPMediaContext *mCtx)
{
    VIPSDLContext *sdlCtx = (VIPSDLContext *)av_mallocz(sizeof(VIPSDLContext));
...
SDL_Window *vip_sdl_create_window(int width , int height)
{
    SDL_Window *window = NULL;
...
int vip_sdl_present_frame(VIPSDLContext *sdlCtx , AVFrame *pFrame){
...
int vip_sdl_play_audio(struct VIPMediaContext *mCtx , VIPSDLContext *sdlCtx)
{
    ...
    wantedSpec.callback = vip_sdl_audio_callback;
